...
public:
	...
	void rotate_left() { state = rotated::left; } 		// Повернуть влево
	void rotate_right() { state = rotated::right; }  	// Повернуть вправо
};


class reflectable : virtual public shape { // Фигуры, пригодные

protected:				     // к зеркальному отражению
...
class rectangle : public rotatable, Exc {      // ==== Прямоугольник ====
	/* nw -------- n ------ ne
	   |		       |
	   |		       |
	   w	         c         e
