...
struct cp_context {
    ...
    // Конструктор: сразу фиксируем время начала и файлы
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        std::cout << "Usage: ./linux <src> <dst> <block_size_kb> <max_ops>" << std::endl;
        return 1;
    }

    double t = aio_copy(argv[1], argv[2], atoi(argv[3]) * 1024, atoi(argv[4]));
